    ...
    def transport(request):
        if request.method == "GET":
            return httpx.Response(200, json={
                "status": "in_progress", "assigned_agent": "engineer",
                "execution_cycle": 2, "execution_generation": 7,
            })
        requests.append(json.loads(request.content))
        if len(requests) == 1:
            raise httpx.ReadTimeout("response lost", request=request)
        return httpx.Response(200, json={
            "attempt_id": attempt_id, "execution_cycle": 2,
            "execution_generation": 8, "agent_name": "engineer",
        })

    client = httpx.AsyncClient(transport=httpx.MockTransport(transport))
    monkeypatch.setattr(httpx, "AsyncClient", lambda **kwargs: client)
    result = await claim_worker_execution(
        "engineer", {"task_id": "task", "status": "ready"}, attempt_id,
        platform_url="http://platform", office_id="office", security_token="test-token",
    )
...
@pytest.mark.parametrize("task", [
    ...
    {"status": "in_progress", "execution_cycle": 1, "execution_generation": 1, "execution_blocked": True},
])
async def test_invalid_or_unfenced_state_never_claims(monkeypatch, task):
    ...
    with pytest.raises(ExecutionClaimError):
        await claim_worker_execution(
            "engineer", {"task_id": "task"}, "attempt",
            platform_url="http://platform", office_id="office", security_token="token",
        )
    assert methods == ["GET"]
...
async def test_claim_failure_never_creates_worker_process(monkeypatch):
    supervisor = AgentSupervisor(".", "office")
    supervisor.set_execution_claimer(AsyncMock(side_effect=ExecutionClaimError("stale")))
    spawn = AsyncMock()
...
def test_execution_event_uses_supervisor_identity_not_worker_payload():
    supervisor = AgentSupervisor(".", "office")
    agent = AgentProcess(
        "engineer", "worker", execution_task_id="task", execution_cycle=2,
        execution_generation=5, execution_attempt_id="trusted-attempt",
    )
    event = supervisor._execution_event(agent, {"type": "progress", "_caller": {"attempt_id": "spoofed"}})
...
def test_local_script_fence_rejects_spoofed_role_or_stale_identity(role):
    supervisor = AgentSupervisor(".", "office")
    agent = AgentProcess(
        "engineer", "worker", process=MagicMock(returncode=None),
        current_task_id="task", execution_cycle=2, execution_generation=5,
        execution_attempt_id="attempt", state=AgentState.WORKING,
    )
    supervisor._agents["engineer"] = agent
    caller = {
        "agent_name": "engineer", "role": role, "task_id": "task",
...
async def test_script_validation_replays_original_claim_and_never_refreshes_owner(monkeypatch):
    requests = []
    caller = {
        "agent_name": "reviewer", "role": "worker", "task_id": "task",
        "attempt_id": "attempt", "execution_cycle": 2, "execution_generation": 5,
        "expected_assigned_agent": "original-executor", "task_mode": "review",
    }
    ...
    assert requests == [{
        "attempt_id": "attempt", "execution_cycle": 2,
        "expected_execution_generation": 4, "expected_assigned_agent": "original-executor",
        "expected_review_retry_epoch": 0,
        "execution_mode": "review",
    }]


async def test_script_validation_refuses_cleared_receipt_even_if_generation_unchanged(monkeypatch):
    caller = {
        "agent_name": "engineer", "role": "worker", "task_id": "task",
        "attempt_id": "attempt", "execution_cycle": 2, "execution_generation": 5,
        "expected_assigned_agent": "engineer", "task_mode": "execute",
    }
    client = httpx.AsyncClient(transport=httpx.MockTransport(lambda request: httpx.Response(400, json={"detail": "stale"})))
    monkeypatch.setattr(httpx, "AsyncClient", lambda **kwargs: client)
