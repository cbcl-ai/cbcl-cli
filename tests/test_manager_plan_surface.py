...
def _envelope_with(env: dict) -> dict:
    ...
    import importlib
    import src._agent_image._mcp_backend as backend
    try:
        with mock.patch.dict(os.environ, env, clear=False):
            importlib.reload(backend)
            return backend._caller_envelope()
    finally:
        importlib.reload(backend)
...
def test_manager_caller_is_stamped_manager_when_task_mode_manager() -> None:
    ...
    assert _envelope_with(env) == {"agent_name": "manager", "role": "manager", "task_mode": "manager"}
...
def test_worker_caller_unchanged() -> None:
    ...
    assert _envelope_with(env) == {"agent_name": "analyst", "role": "worker", "task_mode": "execute"}


def test_planner_caller_unchanged() -> None:
    env = {"AGENT_NAME": "planner", "TASK_MODE": "execute"}
    assert _envelope_with(env) == {"agent_name": "planner", "role": "worker", "task_mode": "execute"}
