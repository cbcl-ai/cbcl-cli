...
class ScriptRunner:
    ...
    # -----------------------------------------------------------------
    # Global periodic sweep — fallback for in-container MCP runs
    # -----------------------------------------------------------------
    #
    # The host-side ``monitor_all`` loop only scans outboxes / status
    # files for executions tracked in ``self._active``. Agent-triggered
    # scripts that run via the in-container MCP ``script_execute`` tool
    # have NO host-side ``_Execution`` tracked, so their
    # ``.outbox/notify-*.json`` drops + ``executions/*/status.json``
    # rows never reach the Manager / backend if the in-container POST
    # to the host tool proxy silently fails (network blip, UFW rule,
    # daemon restart between script completion and POST).
    #
    # The sweep below runs every 30 s independently of host-active
    # executions and:
    #   1. Walks every script's ``.outbox/`` to deliver missed
    #      ``cubicle.notify_manager()`` drops.
    #   2. Walks every script's ``executions/*/status.json`` to find
    #      terminal rows that haven't been reported to the backend
    #      yet (marker file: ``.reported`` next to status.json).
    #
    # Bounded cost — most offices have 5-20 scripts × ~1-5 execution
    # dirs each, so a single sweep is sub-100ms of disk I/O. Skip
    # silently if config/manager wiring is incomplete.

    _SWEEP_INTERVAL_S = 30
    _SWEEP_REPORTED_MARKER = ".reported"

    async def global_sweep(self) -> None:
        """Background loop. Runs forever until cancelled.

        Stand-alone fallback for outbox + execution-status delivery
        when the in-container MCP path can't reach the host tool proxy.
        """
        ...
        while True:
            try:
                ...
                if not scripts_root.is_dir():
                    continue
                await self._sweep_outboxes(scripts_root)
                await self._sweep_unreported_executions(scripts_root)
            except asyncio.CancelledError:
                raise
            except Exception:
                logger.exception("Global script sweep failed — continuing")

    async def _sweep_outboxes(self, scripts_root: Path) -> None:
        """Deliver any unprocessed ``notify_manager`` drops across
        all scripts. No-op if config_store / manager not wired
        (test fixtures, early-startup race)."""
        ...
        for script_dir in scripts_root.iterdir():
            ...
            if not outbox.is_dir():
                continue
            # Has anything to deliver? Check for un-archived JSON files.
            has_pending = any(
            ...
            try:
                ...
                if dispatched:
                    logger.info(
                        "global_sweep: delivered %d outbox notification(s) for %s",
                        dispatched, script_dir.name,
                    )
            except Exception:  # noqa: BLE001
                logger.exception(
                    "global_sweep: outbox dispatch for %s failed",
                    script_dir.name,
                )

    async def _sweep_unreported_executions(self, scripts_root: Path) -> None:
        """Re-publish any terminal ``status.json`` rows that haven't
        been reported to the backend yet.

        Marks each row with a ``.reported`` sentinel after a
        successful publish so we don't re-publish on every sweep.
        Without this, the user's "Execution History" tab stays empty
        for AI-test runs whose in-container ``_report_status_to_backend``
        POST silently failed (transport hiccup, daemon restart, etc.)."""
        ...
        for script_dir in scripts_root.iterdir():
            ...
            if not exec_root.is_dir():
                continue
            for exec_dir in exec_root.iterdir():
                if not exec_dir.is_dir():
                    continue
                status_file = exec_dir / "status.json"
                marker = exec_dir / self._SWEEP_REPORTED_MARKER
                if not status_file.is_file() or marker.is_file():
                    continue
                try:
                ...
                # Only terminal rows resync — running ones still emit
                # via the live monitor / in-container path and would
                # confuse the dispatcher with mid-execution status
                # transitions.
                ...
                try:
                    await notify_completion(
                        ...
                        triggered_by=status_data.get("triggered_by") or "agent",
                        started_at_iso=status_data.get("started_at", ""),
                        process_returncode=status_data.get("exit_code"),
                        status=status,
                        duration=float(status_data.get("duration_seconds") or 0),
                        error_message=status_data.get("error_message"),
                        progress=status_data.get("progress") or {},
                        router=self._router,
                        cron_id=status_data.get("cron_id"),
                    )
                    # Mark as reported so the next sweep skips. Best-
                    # effort write — if it fails we just re-publish
                    # the row next tick, which the backend deduplicates
                    # on (script_id, execution_id).
                    try:
                        marker.write_text("")
                    except OSError:
                        logger.warning(
                            "global_sweep: failed to write .reported marker "
                            "for %s/%s — will re-publish next tick",
                            script_dir.name, exec_dir.name,
                        )
                    logger.info(
                        "global_sweep: resynced %s execution %s (%s)",
                        script_dir.name, exec_dir.name, status,
                    )
                except Exception:  # noqa: BLE001
                    logger.exception(
                        "global_sweep: resync of %s/%s failed",
                        script_dir.name, exec_dir.name,
                    )
