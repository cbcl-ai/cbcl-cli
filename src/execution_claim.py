...
async def validate_worker_execution(
    task_id: str, caller: dict, *, platform_url: str,
    office_id: str, security_token: str,
) -> None:
    generation = caller.get("execution_generation")
    cycle = caller.get("execution_cycle")
    epoch = caller.get("review_retry_epoch", 0)
    if (
        caller.get("role") != "worker" or caller.get("task_id") != task_id
        ...
        or isinstance(cycle, bool) or not isinstance(cycle, int) or cycle < 0
        or isinstance(epoch, bool) or not isinstance(epoch, int) or epoch < 0
    ):
        raise ExecutionClaimError("Task script launch requires a complete execution identity")
    request = {
        "attempt_id": caller["attempt_id"], "execution_cycle": cycle,
    ...
    if not isinstance(receipt, dict) or any(
        ...
        for key in ("attempt_id", "execution_cycle", "execution_generation", "agent_name")
    ) or receipt.get("review_retry_epoch", 0) != epoch:
        raise ExecutionClaimError("Task script launch refused: execution receipt is stale")


async def claim_worker_execution(
    agent_name: str, task_data: dict, attempt_id: str, *,
    ...
    expected_status = {"execute": "in_progress", "review": "review", "triage": "blocked"}[mode]
    task_url = f"{platform_url.rstrip('/')}/api/offices/{office_id}/tasks/{task_id}"
    async with httpx.AsyncClient(timeout=10.0, headers=auth_headers(security_token)) as client:
        response = await client.get(task_url)
        response.raise_for_status()
        task = response.json()
        if not isinstance(task, dict) or task.get("status") != expected_status or task.get("execution_blocked"):
            raise ExecutionClaimError("Task execution admission changed; reconcile before retrying")
        cycle = task.get("execution_cycle")
        generation = task.get("execution_generation")
        epoch = task.get("review_retry_epoch", 0)
        if any(isinstance(value, bool) or not isinstance(value, int) or value < 0 for value in (cycle, generation, epoch)):
            raise ExecutionClaimError("Backend execution fencing is unavailable; upgrade the platform before dispatch")
        request = {
        ...
        for retry_index in range(3):
            try:
                response = await client.post(f"{task_url}/execution-attempts/claim", json=request)
                ...
                if (
                    ...
                    raise ExecutionClaimError("Backend returned an inconsistent execution claim")
                return {**claim, "expected_assigned_agent": request["expected_assigned_agent"], "review_retry_epoch": epoch}
