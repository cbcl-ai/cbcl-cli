...
_MODE_INSTRUCTIONS = {
    "specify": (
        ...
        "designs (the plan owns HOW). Aim for ≤1–2k authored tokens excluding "
    ...
    "materialize": (
        ...
        "in backlog/ready/blocked for a never-executed task in this consult's "
        "scope; previously executed blocked work needs Manager repair. "
        "Never rewrite in_progress/review work. "
        ...
        "effort_hint:'xhigh' for direct builds and focused refinements. "
        "Use ultracode only for independent implementation branches that "
        "shorten the critical path; do not add review fan-out. A scoped "
        "task is not automatically complex. The backend adds a size_note past 3 tasks in a "
    ...
    "verify": (
        ...
        "artifacts. Reconcile every task's acceptance criteria using its "
        "accepted review evidence; independently inspect the integrated "
        "milestone, cross-task interfaces and changed/high-risk behavior. "
        "Reuse only inspectable trustworthy automated results tied to the "
        "exact revision and relevant environment/inputs. Re-run missing, "
        "stale, doubtful or explicitly independent checks; task status or "
        ...
        "then call passed=false (coverage_map optional on fail). Rework has no count limit. "
        "Flag recurring gaps for Manager assessment; continue actionable rework. Pending spec approval "
        ...
        "accepted verdict whenever the backend can record it. A missing verdict "
        "triggers bounded recovery; inspect existing evidence before repeating checks. If a "
        "PASS is refused, correct only evidence-backed chip/coverage omissions "
        "and call again — do not stop on a refused verdict. Never repeat the same "
        "rejected PASS or mark an unproven chip done; unresolved approval/Stop "
        "gates use the blocked-prerequisite path above. If a transient backend "
        "failure prevents recording ANY verdict, report the actual error in your "
        "completion and end for bounded recovery; do not retry indefinitely."
    ),
}
