...
CONFIGURATION_TOOLS = [
    {
        ...
        "description": "Read current Office instructions, Workstream context, or a custom Agent's instructions before proposing edits. Returns recent proposal decisions and feedback. Pass proposal_id to inspect an existing proposal, including its exact edits. READ-ONLY. Do not use for credentials, permissions, built-in Manager prompts, CI files or runtime settings. Use list_agents/get_board for target IDs; office target_id defaults to this office.",
        ...
        "inputSchema": {"type": "object", "properties": {
            ...
            "changes": {"type": "array", "minItems": 1, "maxItems": 12, "items": {
                "type": "object", "properties": {
                    "target": {"type": "string", "enum": ["office", "workstream", "agent"]},
                    "target_id": {"type": "string"},
                    "field": {"type": "string", "enum": ["claude_md_content", "context_notes", "system_prompt", "role_description"]},
                    "before": {"type": ["string", "null"]},
                    "after": {"type": ["string", "null"]},
                    "reason": {"type": "string"},
                }, "required": ["target", "target_id", "field", "before", "after", "reason"], "additionalProperties": False,
            }},
        }, "required": ["title", "rationale", "evidence", "topic", "changes"], "additionalProperties": False},
    },
]

# Parameter intent is part of the model-facing contract, not inferred from names.
_PARAMETER_DESCRIPTIONS = {
    "target": "Instruction owner: office (default), workstream, or custom agent.",
    "target_id": "Target UUID from live context/roster; office defaults to this office.",
