...
generated by the wizard and the Settings Generate/Improve buttons now
share ONE contract instead of two contradictory prompts.

**Office instructions**

- `OFFICE_INSTRUCTIONS_CONTRACT` — a single shared contract composed
  into both generation prompts: Manager-only audience truth, a
  2-4-section menu (Mission · Domain Knowledge · Roster shape ·
  Conventions · optional Quality bar), a forbidden-headers list naming
  each platform owner (no more restated workspace paths, escalation
  taxonomy, comms norms, task lifecycle), and a hard budget — target
  900-2,500 characters, ceiling 4,500 (the save cap is 16,000; the
  curated department templates average ~1,100 and are the standard).
- The wizard prompt dropped its 8-mandatory-section / 700-1400-word
  outline; the flows array is the ONLY carrier of workflows.
- Improve mode may SHRINK: consolidation is success, the budget is
  binding, and an over-budget input is a compression job first. The
  current document is now spliced into the prompt inside an injection
  fence like every other user input.
- Oversize safety: generation self-checks against the 16,000-char save
  cap with one budget-aware compression retry; the Settings path then
  errors honestly instead of returning an unsavable draft; the wizard
  path trims at a paragraph boundary with a visible marker — the silent
  mid-sentence truncation at apply is gone (backend half in v4.6.0).
